...
// TODO: LED->backlight renames/reworks. (This driver was adapted from LED one.)
...
static void omap_pwm_led_set_pwm_cycle(struct omap_pwm_led *led, int cycle)
{
	...
	if (cycle < 0)
		cycle = 0;
	if (led->pdata->bkl_max && cycle > led->pdata->bkl_max)
		cycle = led->pdata->bkl_max;
...
static void omap_pwm_led_set(struct omap_pwm_led *led, int brightness)
{
	if (brightness != 0) {
		omap_pwm_led_power_on(led);
		omap_pwm_led_set_pwm_cycle(led, brightness);
	} else {
...
{
	struct omap_pwm_led *led = dev_get_drvdata(&bl->dev);
	...
	omap_pwm_led_set(led, brightness);
...
static int omap_pwm_led_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "OMAP PWM LCD backlight (%s) at GP timer %d/%d\n",
	...
	props.max_brightness = (pdata->bkl_max) ? pdata->bkl_max : 255;
	led->bl_dev = backlight_device_register(dev_name(&pdev->dev), &pdev->dev, led,
				       &pwm_backlight_ops, &props);
	if (IS_ERR(led->bl_dev)) {
		dev_err(&pdev->dev, "failed to register backlight\n");
		ret = PTR_ERR(led->bl_dev);
		goto error_bl_dev;
	}

	// TODO:
	led->bl_dev->props.brightness = pdata->bkl_max * 2 / 3;
	backlight_update_status(led->bl_dev);

	//platform_set_drvdata(pdev, led->bl_dev);
...
error_blink1:
	backlight_device_unregister(led->bl_dev);
error_bl_dev:
...
MODULE_DESCRIPTION("OMAP PWM Backlight driver");
MODULE_LICENSE("GPL");
